...
@router.post("/api/bookslitres")
async def parse_litres_book(input_data: LitresUrlInput) -> LitresBookResponse:
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(str(input_data.url))
            response.raise_for_status()
        except httpx.RequestError:
            raise HTTPException(status_code=400, detail="Не удалось получить данные с Литрес")
        ...
        if not title_elem:
            raise HTTPException(status_code=400, detail="Не удалось найти название книги")
        title = title_elem.text.strip()

        # Получаем авторов
        authors = []
        author_elements = soup.select('.biblio_book_author a')
        if not author_elements:
            author_elements = soup.select('.authors__name')
        if not author_elements:
            raise HTTPException(status_code=400, detail="Не удалось найти автора(ов) книги")
        for author_elem in author_elements:
            author_name = author_elem.text.strip()
            if author_name and not author_name.lower().startswith(('читает', 'исполняет')):
        ...
        image_elem = soup.select_one('img.cover-image')
        if not image_elem or not image_elem.get('src'):
            raise HTTPException(status_code=400, detail="Не удалось найти обложку книги")
        image_url = image_elem['src']
        if not image_url.startswith('http'):
            image_url = f"https:{image_url}"

        return LitresBookResponse(
            title=title,
            authors=authors,
